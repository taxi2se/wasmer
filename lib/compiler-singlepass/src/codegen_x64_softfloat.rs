...
impl<'a> FuncGen<'a> {
    ...
    fn emit_fp_softfloat32_to_softfloat64(
        &mut self,
        f: fn(u32) -> u64,
    ) -> Result<(), CodegenError> {
        let loc_a = self.pop_value_released();
    ...
    fn emit_fp_softfloat64_to_softfloat32(
        &mut self,
        f: fn(u64) -> u32,
    ) -> Result<(), CodegenError> {
        let loc_a = self.pop_value_released();
    ...
    pub fn feed_operator(&mut self, op: Operator) -> Result<(), CodegenError> {
        ...
        match op {
            ...
            Operator::F32Add => {
                ...
                self.emit_fp_binop_softfloat32(softfloat32_add_f32)?
            }
            Operator::F32Sub => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat32(softfloat32_sub_f32)?
            }
            Operator::F32Mul => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat32(softfloat32_mul_f32)?
            }
            Operator::F32Div => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat32(softfloat32_div_f32)?
            }
            Operator::F32Max => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::new(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat32(softfloat32_max_f32)?
            }
            Operator::F32Min => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat32(softfloat32_min_f32)?
            }
            Operator::F32Eq => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat32(softfloat32_eq_f32)?
            }
            Operator::F32Ne => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat32(softfloat32_ne_f32)?
            }
            Operator::F32Lt => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat32(softfloat32_lt_f32)?
            }
            Operator::F32Le => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat32(softfloat32_le_f32)?
            }
            Operator::F32Gt => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat32(softfloat32_gt_f32)?
            }
            Operator::F32Ge => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat32(softfloat32_ge_f32)?
            }
            Operator::F32Nearest => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat32(softfloat32_nearest_f32)?
            }
            Operator::F32Floor => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat32(softfloat32_floor_f32)?
            }
            Operator::F32Ceil => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat32(softfloat32_ceil_f32)?
            }
            Operator::F32Trunc => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat32(softfloat32_trunc_f32)?
            }
            Operator::F32Sqrt => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f32(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat32(softfloat32_sqrt_f32)?
            }
            ...
            Operator::F64Add => {
                ...
                self.emit_fp_binop_softfloat64(softfloat64_add_f64)?
            }
            Operator::F64Sub => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::cncl_f64(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat64(softfloat64_sub_f64)?
            }
            Operator::F64Mul => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::cncl_f64(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat64(softfloat64_mul_f64)?
            }
            Operator::F64Div => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::cncl_f64(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat64(softfloat64_div_f64)?
            }
            Operator::F64Max => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::new(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat64(softfloat64_max_f64)?
            }
            Operator::F64Min => {
                self.fp_stack.pop2()?;
                self.fp_stack
                    .push(FloatValue::new(self.value_stack.len() - 2));
                self.emit_fp_binop_softfloat64(softfloat64_min_f64)?
            }
            Operator::F64Eq => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat64(softfloat64_eq_f64)?
            }
            Operator::F64Ne => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat64(softfloat64_ne_f64)?
            }
            Operator::F64Lt => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat64(softfloat64_lt_f64)?
            }
            Operator::F64Le => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat64(softfloat64_le_f64)?
            }
            Operator::F64Gt => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat64(softfloat64_gt_f64)?
            }
            Operator::F64Ge => {
                self.fp_stack.pop2()?;
                self.emit_fp_cmpop_softfloat64(softfloat64_ge_f64)?
            }
            Operator::F64Nearest => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f64(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat64(softfloat64_nearest_f64)?
            }
            Operator::F64Floor => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f64(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat64(softfloat64_floor_f64)?
            }
            Operator::F64Ceil => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f64(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat64(softfloat64_ceil_f64)?
            }
            Operator::F64Trunc => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f64(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat64(softfloat64_trunc_f64)?
            }
            Operator::F64Sqrt => {
                self.fp_stack.pop1()?;
                self.fp_stack
                    .push(FloatValue::cncl_f64(self.value_stack.len() - 1));
                self.emit_fp_unop_softfloat64(softfloat64_sqrt_f64)?
            }
            ...
            Operator::F64PromoteF32 => {
                ...
                self.emit_fp_softfloat32_to_softfloat64(softfloat32_promote64_f32)?
            }
            Operator::F32DemoteF64 => {
                println!("f64->f32");
                let fp = self.fp_stack.pop1()?;
                self.fp_stack.push(fp.demote(self.value_stack.len() - 1));
                self.emit_fp_softfloat64_to_softfloat32(softfloat64_demote32_f64)?
            }
...
fn _softfloat32_to_float32_t(f: u32) -> float32_t {
    // float32_t { v: f.to_bits() }
    float32_t { v: f }
}

fn _softfloat32_to_f32(f: float32_t) -> f32 {
    f32::from_bits(f.v)
}
...
fn softfloat32_sub_f32(lhs: u32, rhs: u32) -> u32 {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    unsafe { softfloat_sys::f32_sub(lhs, rhs).v }
}
...
fn softfloat32_div_f32(lhs: u32, rhs: u32) -> u32 {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    unsafe { softfloat_sys::f32_div(lhs, rhs).v }
}

fn softfloat32_max_f32(lhs: u32, rhs: u32) -> u32 {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    if unsafe { softfloat_sys::f32_lt(lhs, rhs) } {
        rhs.v
    } else {
        lhs.v
    }
...
fn softfloat32_min_f32(lhs: u32, rhs: u32) -> u32 {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    if unsafe { softfloat_sys::f32_lt(lhs, rhs) } {
        lhs.v
...
fn softfloat32_eq_f32(lhs: u32, rhs: u32) -> bool {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    unsafe { softfloat_sys::f32_eq(lhs, rhs) }
}

fn softfloat32_ne_f32(lhs: u32, rhs: u32) -> bool {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    !unsafe { softfloat_sys::f32_eq(lhs, rhs) }
}

fn softfloat32_lt_f32(lhs: u32, rhs: u32) -> bool {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    unsafe { softfloat_sys::f32_lt(lhs, rhs) }
}

fn softfloat32_le_f32(lhs: u32, rhs: u32) -> bool {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    unsafe { softfloat_sys::f32_le(lhs, rhs) }
}

fn softfloat32_gt_f32(lhs: u32, rhs: u32) -> bool {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    unsafe { softfloat_sys::f32_lt(rhs, lhs) }
}

fn softfloat32_ge_f32(lhs: u32, rhs: u32) -> bool {
    let lhs = _softfloat32_to_float32_t(lhs);
    let rhs = _softfloat32_to_float32_t(rhs);
    unsafe { softfloat_sys::f32_le(rhs, lhs) }
}

fn softfloat32_nearest_f32(src: u32) -> u32 {
    let src = _softfloat32_to_float32_t(src);
    unsafe {
        let ret =
            softfloat_sys::f32_roundToInt(src, softfloat_sys::softfloat_round_near_even, true);
        let _temp = f32::from_bits(ret.v);
        ret.v
    }
}

fn softfloat32_floor_f32(src: u32) -> u32 {
    let src = _softfloat32_to_float32_t(src);
    unsafe {
        let ret = softfloat_sys::f32_roundToInt(src, softfloat_sys::softfloat_round_min, true);
        let _temp = f32::from_bits(ret.v);
        ret.v
    }
}

fn softfloat32_ceil_f32(src: u32) -> u32 {
    let src = _softfloat32_to_float32_t(src);
    unsafe {
        let ret = softfloat_sys::f32_roundToInt(src, softfloat_sys::softfloat_round_max, true);
        let _temp = f32::from_bits(ret.v);
        ret.v
    }
}

fn softfloat32_trunc_f32(src: u32) -> u32 {
    let src = _softfloat32_to_float32_t(src);
    unsafe {
        let ret = softfloat_sys::f32_roundToInt(src, softfloat_sys::softfloat_round_minMag, true);
        let _temp = f32::from_bits(ret.v);
        ret.v
    }
}

fn softfloat32_sqrt_f32(src: u32) -> u32 {
    let src = _softfloat32_to_float32_t(src);
    unsafe {
        let ret = softfloat_sys::f32_sqrt(src);
        let _temp = f32::from_bits(ret.v);
        ret.v
    }
}

fn softfloat32_promote64_f32(src: u32) -> u64 {
    let src = _softfloat32_to_float32_t(src);
    unsafe {
        let ret = softfloat_sys::f32_to_f64(src);
        let _temp = f64::from_bits(ret.v);
        ret.v
    }
}

use softfloat_sys::float64_t;
fn _softfloat64_to_float64_t(f: u64) -> float64_t {
    // float64_t { v: f.to_bits() }
    float64_t { v: f }
}

fn _softfloat64_to_f64(f: float64_t) -> f64 {
    f64::from_bits(f.v)
}

fn softfloat64_add_f64(lhs: u64, rhs: u64) -> u64 {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_add(lhs, rhs).v }
}

fn softfloat64_sub_f64(lhs: u64, rhs: u64) -> u64 {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_sub(lhs, rhs).v }
}

fn softfloat64_mul_f64(lhs: u64, rhs: u64) -> u64 {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_mul(lhs, rhs).v }
}

fn softfloat64_div_f64(lhs: u64, rhs: u64) -> u64 {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_div(lhs, rhs).v }
}

fn softfloat64_max_f64(lhs: u64, rhs: u64) -> u64 {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    if unsafe { softfloat_sys::f64_lt(lhs, rhs) } {
        rhs.v
    } else {
        lhs.v
    }
...
fn softfloat64_min_f64(lhs: u64, rhs: u64) -> u64 {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    if unsafe { softfloat_sys::f64_lt(lhs, rhs) } {
        lhs.v
...
fn softfloat64_eq_f64(lhs: u64, rhs: u64) -> bool {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_eq(lhs, rhs) }
}

fn softfloat64_ne_f64(lhs: u64, rhs: u64) -> bool {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    !unsafe { softfloat_sys::f64_eq(lhs, rhs) }
}

fn softfloat64_lt_f64(lhs: u64, rhs: u64) -> bool {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_lt(lhs, rhs) }
}

fn softfloat64_le_f64(lhs: u64, rhs: u64) -> bool {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_le(lhs, rhs) }
}

fn softfloat64_gt_f64(lhs: u64, rhs: u64) -> bool {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_lt(rhs, lhs) }
}

fn softfloat64_ge_f64(lhs: u64, rhs: u64) -> bool {
    let lhs = _softfloat64_to_float64_t(lhs);
    let rhs = _softfloat64_to_float64_t(rhs);
    unsafe { softfloat_sys::f64_le(rhs, lhs) }
}

fn softfloat64_nearest_f64(src: u64) -> u64 {
    let src = _softfloat64_to_float64_t(src);
    unsafe {
        let ret =
            softfloat_sys::f64_roundToInt(src, softfloat_sys::softfloat_round_near_even, true);
        let _temp = f64::from_bits(ret.v);
        ret.v
    }
}

fn softfloat64_floor_f64(src: u64) -> u64 {
    let src = _softfloat64_to_float64_t(src);
    unsafe {
        let ret = softfloat_sys::f64_roundToInt(src, softfloat_sys::softfloat_round_min, true);
        let _temp = f64::from_bits(ret.v);
        ret.v
    }
}

fn softfloat64_ceil_f64(src: u64) -> u64 {
    let src = _softfloat64_to_float64_t(src);
    unsafe {
        let ret = softfloat_sys::f64_roundToInt(src, softfloat_sys::softfloat_round_max, true);
        let _temp = f64::from_bits(ret.v);
        ret.v
    }
}

fn softfloat64_trunc_f64(src: u64) -> u64 {
    let src = _softfloat64_to_float64_t(src);
    unsafe {
        let ret = softfloat_sys::f64_roundToInt(src, softfloat_sys::softfloat_round_minMag, true);
        let _temp = f64::from_bits(ret.v);
        ret.v
    }
}

fn softfloat64_sqrt_f64(src: u64) -> u64 {
    let src = _softfloat64_to_float64_t(src);
    unsafe {
        let ret = softfloat_sys::f64_sqrt(src);
        let _temp = f64::from_bits(ret.v);
        ret.v
    }
}

fn softfloat64_demote32_f64(src: u64) -> u32 {
    let src = _softfloat64_to_float64_t(src);
    unsafe {
        let ret = softfloat_sys::f64_to_f32(src);
        let _temp = f32::from_bits(ret.v);
        ret.v
    }
}
