...
fn _eq_f32(lhs: f32, rhs: f32) -> i32 {
    unsafe {
        let lhs = _mm_set1_ps(lhs);
        let rhs = _mm_set1_ps(rhs);
        let ret = _mm_cmp_ps::<_CMP_EQ_UQ>(lhs, rhs);
        let ret = _mm_extract_ps::<0>(ret);
        ret & 1
    }
}
...
fn _ne_f32(lhs: f32, rhs: f32) -> i32 {
    unsafe {
        let lhs = _mm_set1_ps(lhs);
        let rhs = _mm_set1_ps(rhs);
        let ret = _mm_cmp_ps::<_CMP_NEQ_OQ>(lhs, rhs);
        let ret = _mm_extract_ps::<0>(ret);
        ret & 1
    }
}
...
fn _lt_f32(lhs: f32, rhs: f32) -> i32 {
    unsafe {
        let lhs = _mm_set1_ps(lhs);
        let rhs = _mm_set1_ps(rhs);
        let ret = _mm_cmp_ps::<_CMP_LT_OQ>(lhs, rhs);
        let ret = _mm_extract_ps::<0>(ret);
        ret & 1
    }
}
...
fn _le_f32(lhs: f32, rhs: f32) -> i32 {
    unsafe {
        let lhs = _mm_set1_ps(lhs);
        let rhs = _mm_set1_ps(rhs);
        let ret = _mm_cmp_ps::<_CMP_LE_OQ>(lhs, rhs);
        let ret = _mm_extract_ps::<0>(ret);
        ret & 1
    }
}
...
fn _gt_f32(lhs: f32, rhs: f32) -> i32 {
    unsafe {
        let lhs = _mm_set1_ps(lhs);
        let rhs = _mm_set1_ps(rhs);
        let ret = _mm_cmp_ps::<_CMP_GT_OQ>(lhs, rhs);
        let ret = _mm_extract_ps::<0>(ret);
        ret & 1
    }
}
...
fn _ge_f32(lhs: f32, rhs: f32) -> i32 {
    unsafe {
        let lhs = _mm_set1_ps(lhs);
        let rhs = _mm_set1_ps(rhs);
        let ret = _mm_cmp_ps::<_CMP_GE_OQ>(lhs, rhs);
        let ret = _mm_extract_ps::<0>(ret);
        ret & 1
    }
}
...
fn _nearest_f32(src: f32) -> f32 {
    unsafe {
        ...
        let ret = _mm_round_ps::<_MM_FROUND_TO_NEAREST_INT>(input);
        let ret = f32::from_bits(_mm_extract_ps::<0>(ret) as u32);
        ret
    }
}
fn _floor_f32(src: f32) -> f32 {
    unsafe {
        let input = _mm_set1_ps(src);
        let ret = _mm_round_ps::<_MM_FROUND_TO_NEG_INF>(input);
        let ret = f32::from_bits(_mm_extract_ps::<0>(ret) as u32);
        ret
    }
}
fn _ceil_f32(src: f32) -> f32 {
    unsafe {
        let input = _mm_set1_ps(src);
        let ret = _mm_round_ps::<_MM_FROUND_TO_POS_INF>(input);
        let ret = f32::from_bits(_mm_extract_ps::<0>(ret) as u32);
        ret
    }
}
fn _trunc_f32(src: f32) -> f32 {
    unsafe {
        let input = _mm_set1_ps(src);
        let ret = _mm_round_ps::<_MM_FROUND_TO_ZERO>(input);
        let ret = f32::from_bits(_mm_extract_ps::<0>(ret) as u32);
        ret
    }
}
...
#[test]
#[cfg(all(feature = "singlepass", feature = "softfloat"))]
fn test_compiler_singlepass_softfloat32() -> Result<(), Box<dyn std::error::Error>> {
    main()
}
