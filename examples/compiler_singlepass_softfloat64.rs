...
fn _eq_f64(lhs: f64, rhs: f64) -> i32 {
    unsafe {
        let lhs = _mm_set1_pd(lhs);
        let rhs = _mm_set1_pd(rhs);
        let ret = _mm_cmp_pd::<_CMP_EQ_UQ>(lhs, rhs);
        let ret = _mm_castpd_si128(ret);
        let ret = _mm_extract_epi64::<0>(ret);
        ret as i32 & 1
    }
}
...
fn _ne_f64(lhs: f64, rhs: f64) -> i32 {
    unsafe {
        let lhs = _mm_set1_pd(lhs);
        let rhs = _mm_set1_pd(rhs);
        let ret = _mm_cmp_pd::<_CMP_NEQ_UQ>(lhs, rhs);
        let ret = _mm_castpd_si128(ret);
        let ret = _mm_extract_epi64::<0>(ret);
        ret as i32 & 1
    }
...
fn _lt_f64(lhs: f64, rhs: f64) -> i32 {
    unsafe {
        let lhs = _mm_set1_pd(lhs);
        let rhs = _mm_set1_pd(rhs);
        let ret = _mm_cmp_pd::<_CMP_LT_OQ>(lhs, rhs);
        let ret = _mm_castpd_si128(ret);
        let ret = _mm_extract_epi64::<0>(ret);
        ret as i32 & 1
    }
...
fn _le_f64(lhs: f64, rhs: f64) -> i32 {
    unsafe {
        let lhs = _mm_set1_pd(lhs);
        let rhs = _mm_set1_pd(rhs);
        let ret = _mm_cmp_pd::<_CMP_LE_OQ>(lhs, rhs);
        let ret = _mm_castpd_si128(ret);
        let ret = _mm_extract_epi64::<0>(ret);
        ret as i32 & 1
    }
...
fn _gt_f64(lhs: f64, rhs: f64) -> i32 {
    unsafe {
        let lhs = _mm_set1_pd(lhs);
        let rhs = _mm_set1_pd(rhs);
        let ret = _mm_cmp_pd::<_CMP_GT_OQ>(lhs, rhs);
        let ret = _mm_castpd_si128(ret);
        let ret = _mm_extract_epi64::<0>(ret);
        ret as i32 & 1
    }
...
fn _ge_f64(lhs: f64, rhs: f64) -> i32 {
    unsafe {
        let lhs = _mm_set1_pd(lhs);
        let rhs = _mm_set1_pd(rhs);
        let ret = _mm_cmp_pd::<_CMP_GE_OQ>(lhs, rhs);
        let ret = _mm_castpd_si128(ret);
        let ret = _mm_extract_epi64::<0>(ret);
        ret as i32 & 1
    }
...
fn _nearest_f64(src: f64) -> f64 {
    unsafe {
        ...
        let ret = _mm_round_pd::<_MM_FROUND_TO_NEAREST_INT>(input);
        _mm_cvtsd_f64(ret)
    }
}
fn _floor_f64(src: f64) -> f64 {
    unsafe {
        let input = _mm_set1_pd(src);
        let ret = _mm_round_pd::<_MM_FROUND_TO_NEG_INF>(input);
        _mm_cvtsd_f64(ret)
    }
}
fn _ceil_f64(src: f64) -> f64 {
    unsafe {
        let input = _mm_set1_pd(src);
        let ret = _mm_round_pd::<_MM_FROUND_TO_POS_INF>(input);
        _mm_cvtsd_f64(ret)
    }
}
fn _trunc_f64(src: f64) -> f64 {
    unsafe {
        let input = _mm_set1_pd(src);
        let ret = _mm_round_pd::<_MM_FROUND_TO_ZERO>(input);
        _mm_cvtsd_f64(ret)
    }
}
...
fn _demote_f64(src: f64) -> f32 {
    unsafe {
        ...
        let ret = f32::from_bits(_mm_extract_ps::<0>(ret) as u32);
        ret
    }
}
...
#[test]
#[cfg(all(feature = "singlepass", feature = "softfloat"))]
fn test_compiler_singlepass_softfloat64() -> Result<(), Box<dyn std::error::Error>> {
    main()
}
